...
from app.services.orientation_detector import correct_orientation
...
def _analyze_yolo(image):
    ...
    # Orientierung korrigieren bevor YOLO laeuft
    resized, orientation = correct_orientation(resized)
    if orientation != 0:
        logger.info(f"Bild um {orientation}° gedreht -> korrigiert.")
