...
def load_image_from_bytes(image_bytes: bytes) -> np.ndarray:
    """Lädt ein Bild aus Bytes in ein OpenCV-Array (mit EXIF-Orientierung)."""
    pil_image = Image.open(io.BytesIO(image_bytes))
    # EXIF-Orientierung anwenden (z.B. Hochformat-Fotos vom Handy)
    pil_image = ImageOps.exif_transpose(pil_image)
    # In RGB konvertieren (falls RGBA, Palette, etc.)
